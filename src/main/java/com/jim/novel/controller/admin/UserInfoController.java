    ...
    public String changePwd(HttpServletRequest request){
        ...
        try {
            ...
            if("0".equals(role)){
                ...
                oldPwd = AuthUtils.getPassword(oldPwd);
                if(oldPwd.equals(me.getPassword())){
                    me.setPassword(AuthUtils.getPassword(request.getParameter("newpassword")));
                    if(userService.updateUserInfoById(me)) {
                        SessionedUser(me);
                        response = new Response(Response.OK_200,"1",null);

                        return response.toJsonString();
                    }else{
                        response = new Response(Response.OK_200,"2",null);
                        return response.toJsonString();
                    }
                }else{
                    response = new Response(Response.OK_200,"3",null);
                    return response.toJsonString();
                }
            }else {
                Admin me = admin();
